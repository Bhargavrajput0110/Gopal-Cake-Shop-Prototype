...
async function run() {
  try {
    ...
    const productCount = await prisma.product.count({ where: { isCustomizable: true }})
    ...
    const order = await prisma.order.create({
      data: {
        ...
        customer: { connect: { id: (await prisma.customer.findFirst())!.id } },
        items: {
          create: [{
    ...
    await prisma.product.delete({ where: { id: customProduct.id }})
