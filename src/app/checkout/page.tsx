          ...
          <div className="lg:col-span-7 space-y-10">
            ...
            <section className="bg-card border border-border/50 rounded-3xl p-6 md:p-8 shadow-sm space-y-6">
              <h2 className="font-serif text-2xl font-bold text-foreground flex items-center gap-3">
                <span className="font-sans text-[10px] uppercase tracking-widest text-secondary font-bold px-2.5 py-1 bg-secondary/10 rounded-full">
                  01
                </span>
                Fulfillment Mode
              </h2>
              ...
              {deliveryType === "PICKUP" ? (
                <div className="space-y-2 pt-2 animate-fadeIn">
                  <label className="text-[10px] font-sans font-bold uppercase tracking-widest text-foreground/50">
                  ...
                  <p className="text-xs font-serif italic text-muted-foreground mt-1">
                    Pick up your fresh cake directly from our kitchen.
                  </p>
                </div>
              ) : (
            ...
            <section className="bg-card border border-border/50 rounded-3xl p-6 md:p-8 shadow-sm space-y-6">
              ...
              <div className="space-y-5">
                {items.map((item, idx) => (
                  <div
                    key={item.cartItemId}
                    className="p-4 bg-muted/20 border border-border/50 rounded-2xl space-y-3"
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex-1">
                        <h4 className="font-serif font-bold text-sm line-clamp-1 pr-4">
                          {item.name}
                        </h4>
                        <div className="flex flex-wrap gap-2 mt-1.5">
                          ...
                          <select
                            ...
                            onChange={(e) =>
                              handleUpdateItem(item, "variant", e.target.value)
                            }
                          ...
                          <select
                            ...
                            onChange={(e) =>
                              handleUpdateItem(item, "flavor", e.target.value)
                            }
                            className="text-[10px] font-sans font-bold bg-background border border-primary/20 rounded px-2 py-0.5 focus:outline-none focus:border-primary text-muted-foreground uppercase tracking-wider cursor-pointer"
                          >
                            ...
                            {flavours.map((f) => (
                              <option key={f.id} value={f.name}>
                                {f.name}{" "}
                                {f.surchargePerHalfKg
                                  ? `(+₹${f.surchargePerHalfKg}/500g)`
                                  : ""}
                              </option>
                            ))}
                          </select>
                          ...
                          <button
                            ...
                            className="flex items-center justify-center gap-1.5 bg-red-50 text-red-500 hover:bg-red-100 hover:text-red-600 transition-colors shrink-0 ml-2 px-2 py-1 rounded border border-red-200"
                            title="Remove from cart"
                          >
                            <Trash className="w-3.5 h-3.5" variant="Bold" />
                            <span className="text-[10px] font-sans font-bold uppercase tracking-widest pt-0.5">Remove</span>
                          </button>
                        </div>
                      </div>
                    </div>
                ...
                {[
                  {
                    ...
                    desc: "Pay 50% now via UPI/Card to confirm your order, pay the rest on delivery/pickup.",
                  },
                  {
                    id: "ONLINE_100",
                    label: "100% Online Payment",
                    desc: "Pay the full amount securely now via UPI or Card.",
                  },
                ].map((method) => (
                  <label
                    key={method.id}
                  ...
                  >
                    <div
                      className={`w-5 h-5 rounded-full border-2 mt-0.5 flex items-center justify-center ${paymentMethod === method.id ? "border-[var(--brand-deep-rose)]" : "border-foreground/30"}`}
                    >
                      {paymentMethod === method.id && (
                        <div className="w-2.5 h-2.5 rounded-full bg-[var(--brand-deep-rose)]" />
                      )}
                    </div>
                    <div>
                      ...
                      <div className="text-xs font-serif italic text-foreground/50 mt-1">
                        {method.desc}
                      </div>
                    </div>
                  </label>
          ...
          <div className="lg:col-span-5">
            <div className="sticky top-32 bg-card border border-border/50 rounded-3xl p-6 md:p-8 shadow-md space-y-6">
                ...
                {items.map((item, idx) => (
                  <div key={idx} className="flex gap-4 items-center">
                    ...
                    <div className="flex-1 min-w-0">
                      <h4 className="font-serif font-bold text-sm text-foreground line-clamp-1 leading-snug">
                        {item.name}
                      </h4>
                      <p className="text-[10px] font-sans text-muted-foreground uppercase font-bold tracking-wider mt-1">
                        {item.variant || "Standard"} • Qty: {item.quantity}
                      </p>
                      {item.flavor &&
                        getFlavourSurcharge(
                          item.flavor,
                          parseWeightToNumber(item.variant || ""),
                        ) > 0 && (
              ...
              <div className="space-y-3 text-base">
                ...
                {deliveryType === "DELIVERY" && (
                  <p className="text-[11px] font-serif italic text-muted-foreground text-right mt-[-4px]">
                ...
                <div className="flex justify-between items-center pt-4 border-t border-border/40">
                  ...
                  <span className="font-serif font-bold text-2xl text-[var(--brand-deep-rose)]">
                    ₹{subtotal}
                  </span>
                </div>
              </div>
