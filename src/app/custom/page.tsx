...
function CustomPageContent() {
  ...
  const product = editingCartItem ? {
    id: editingCartItem.productId || editingCartItem.designId || "edited-item",
    name: editingCartItem.name,
    basePrice: editingCartItem.basePrice || editingCartItem.price || 600,
    price: editingCartItem.price,
    thumbnail: editingCartItem.image,
    isCustom: Boolean(editingCartItem.isCustom) || editingCartItem.name.toLowerCase().includes("custom"),
    isPhotoCake: Boolean(editingCartItem.isPhotoCake) || editingCartItem.name.toLowerCase().includes("photo"),
  } : customProduct;

  const isActuallyCustom = editingCartItem ? (product as any).isCustom : true;
      ...
      <div className="max-w-[1200px] mx-auto px-4 md:px-8">
        ...
        <div className="mb-12 border-b border-border/40 pb-6">
          <span className="text-[10px] font-sans font-bold text-secondary tracking-[0.3em] uppercase block mb-3">
            {editingCartItem ? "Edit Your Item" : "Tailored Experience"}
          </span>
        ...
        >
          <QuickBuyForm 
            ...
            isPhotoCake={(product as any).isPhotoCake}
